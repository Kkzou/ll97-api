# -*- coding: utf-8 -*-
"""
main.py — LL97 Compliance Risk Prediction API  v6.0

Key behaviors:
  - Path A: user provides energy bills -> direct LL97 calculation (no ML)
  - Path B: user provides building features only -> MLP neural network
  - All engineered features (age_x_sqft, severity proxies, etc.) are
    computed LIVE from user inputs, not pulled from feature_means.json.
  - feature_means.json is used only for raw missing values (e.g. occupants,
    seasonal weather the user doesn't know).
  - Asymmetric threshold: if P(high) >= 0.30, predict 'high'.
"""
...
app = FastAPI(title="LL97 Risk API", version="6.0.0")
...
# ---------------------------------------------------
def fill(val, key):
    """Use user value, else dataset median for that feature."""
...
def predict(b: BuildingInput):
    try:
        ...
        # ====================================================
        # Path A — Energy bills provided → direct calculation
        # ====================================================
        has_energy = (
        ...
        # ====================================================
        # Path B — No energy data → ML classification
        # ====================================================
        if MODEL is None:
            raise HTTPException(503, "ML model not available")

        # ---- 1. Resolve raw user inputs (or median fallback) ----
        ...
        # ---- 2. ALWAYS compute engineered features from above (never from MEANS) ----
        ...
        # Building age bucket — same logic as training
        ...
        # All seasonal weather the user can't easily provide → median fallback
        row = {
            "sqft":                            sqft,
            ...
            "occupants":                       occupants,
            "energystarscore":                 MEANS.get("energystarscore", 50),
            ...
            # Engineered — always live-computed
        ...
        # ---- 5. Predict with asymmetric threshold ----
        ...
        return {
            ...
            "note": f"Classification with asymmetric threshold (P(high) >= {HIGH_THRESHOLD} -> high). For exact emissions and penalty estimates, provide your annual energy bills.",
        }
